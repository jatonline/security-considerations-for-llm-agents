...
other-links:
  - text: PowerPoint slides
    href: "slides/Security considerations for LLM agents.pptx"
  - text: GitHub source
    href: https://github.com/jatonline/security-considerations-for-llm-agents
resources:
  - slides/Security considerations for LLM agents.pptx
---
...
![](slides/images/slide01.png){fig-alt="Security considerations for LLM agents. James Thomas, Jean Golding Institute, 15 July 2026."}
...
![](slides/images/slide02.png){fig-alt="Overview. Examples of what can go wrong and why: 1. Private data sent to the LLM provider. 2. When the agent makes a mistake. 3. Prompt injection. Mitigations and protection strategies: 1. Disabling model training. 2. Software engineering best practices. 3. Permissions. 4. Sandboxes. 5. Someone else's machine."}
...
![](slides/images/slide03.png){fig-alt="Private data sent to the LLM provider. All files read by the agent and all outputs from shell commands are uploaded to the web. If your data cannot leave your infrastructure, this is a problem."}
...
![](slides/images/slide05.png){fig-alt="Screenshot of a chat interface with the prompt \"Explore this repo and explain what it does\" dated 25 June 2026, VS Code + GitHub Copilot, GPT-5.3-Codex, ~default settings."}
...
![](slides/images/slide07.png){fig-alt="Continuation of the chat interface: main.py then plots numeric columns. Notes that the script reads the CSV, visualises numeric fields like telephone and credit card numbers, and that display depends on environment. Warns that data.csv contains personal-looking fields and should be treated as sensitive data."}
...
![](slides/images/slide08.png){fig-alt="Our private data was sent to the cloud. Screenshots of the chat interface showing the read tool running on main.py and data.csv, and a note stating that data.csv contains personal-looking fields (name, address, phone, email, credit card) and should be treated as sensitive even if synthetic."}
...
![](slides/images/slide11.png){fig-alt="When the agent makes a mistake. Agents take actions or call external tools by outputting specially formatted responses which their harness recognises. This can sometimes go wrong."}
...
![](slides/images/slide13.png){fig-alt="Reports of accidental file deletion. Examples of bugs: a forum post about \"Critical Data Loss Issue in Codex App for Windows - Agent Executed File Deletion Outside Project Directory\", a GitHub issue \"[BUG] CRITICAL: Claude Code executed rm -rf deleting entire home directory\", and other examples seen in the wild: \"git reset --hard\", \"rm -rf *\", and \"rm -rf .git\"."}
...
![](slides/images/slide14.png){fig-alt="Prompt injection. All data read by the agent is concatenated to the prompt that the LLM sees. If the data contains something that looks like a prompt, the agent may follow this new instruction."}
...
![](slides/images/slide16.png){fig-alt="Examples of data exfiltration. A Hacker News article: \"New Attacks Trick OpenClaw AI Agent Into Running Code and Leaking Secrets\", describing how AI agents can be induced to run attacker-controlled code or leak sensitive data via normal-looking inputs."}
...
![](slides/images/slide17.png){fig-alt="Protection strategies. Follow software engineering best practices. Prevent the agent from: Reading files/data, modifying or deleting files, running commands or tools, and accessing URLs and services."}
...
![](slides/images/slide20.png){fig-alt="Sandboxes: built-in. Table comparing \"Sandboxed Bash tool\" isolates bash commands and child processes, no Docker required, minimal setup; \"Sandbox runtime\" isolates the whole Claude Code process including file tools, MCP servers, hooks, no Docker, low setup."}
...
![](slides/images/slide23.png){fig-alt="Sandboxes: containers. Table comparing: \"Dev container\" full development environment, requires Docker, medium setup; \"Custom container\" full development environment, requires Docker, medium to high setup; \"Virtual machine\" full operating system, no Docker, high setup."}
...
![](slides/images/slide25.png){fig-alt="Dev container. Screenshot of a Visual Studio Code window inside the dev container. Shows a terminal listing the home directory (which is empty) and the /workspace directory (which contains only the files in the dev container)."}
...
![](slides/images/slide26.png){fig-alt="Custom containers. Roll your own docker (or equivalent) or use a tool like Docker Sandboxes."}
...
![](slides/images/slide27.png){fig-alt="Someone else's machine. Table with \"Claude Code on the web\" full operating system hosted by Anthropic, no Docker, no setup beyond subscription and GitHub. Followed by a screenshot of the page \"Get started with Claude Code on the web\"."}
...
![](slides/images/slide28.png){fig-alt="My opinions: 1. (Disabling model training), 2. Follow software engineering best practices, 3. Don't rely on built-in permissions or sandboxes, 4. Use a container sandbox (or a VM, or someone else's computer)."}
...
![](slides/images/slide29.png){fig-alt="Thank you slide. James Thomas, Jean Golding Institute."}
